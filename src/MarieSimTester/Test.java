...
public class Test {
	static final long PollInterval = 2000; // Poll the simulator's input
											// register after each interval
											// (msec)
	...
	private enum IOMode {
		hex(MarieSim.HEX), ascii(MarieSim.ASCII), dec(MarieSim.DEC);
		...
		if (dir.isDirectory()) {
			FileFilter filter = new FileFilter() {
				...
				public boolean accept(File pathname) {
					if (pathname.getName().endsWith(".mas"))
						return true;
					return false;
				}
			};
	...
	private boolean isMarieSimHalted() {
		...
		if (instance.machineState == MarieSim.MARIE_HALTED_ABNORMAL)
			return true;
		if (instance.machineState == MarieSim.MARIE_HALTED_NORMAL)
			return true;
		if (instance.machineState == MarieSim.MARIE_HALTED_BY_USER)
			return true;
		return false;
	}

	private boolean isMarieSimRequestingInput() {
		// The simulator makes the input register editable if and only if it
		// needs input.
	...
	private void singleRun(File mexfile, String input, String expectedOutput, IOMode inputMode, IOMode outputMode,
			int maxPolls) {
		...
		// If multiple inputs are provided within a single runs, the inputs are
		// expected
		...
		timer.scheduleAtFixedRate(new TimerTask() {
			public void run() {
				...
				if (pollCount <= maxPolls && !isMarieSimHalted()) {
					// If tester provided input, then supply it to the simulator
					// upon request.
	...
	private void multiRuns(File mexfile, String inputs[], String expectedOutputs[], IOMode inputMode, IOMode outputMode,
			int maxPollsPerRun) {
		if (mexfile == null || !mexfile.exists())
			return;
		if (instance == null || robot == null)
				...
				if (runCount < inputs.length) {
					if (isMarieSimHalted()) {
						...
						if (pollCount <= maxPollsPerRun) {
							// If tester provided input, then supply it to the
		...
		}, PollInterval * 1, // runs first occurrence after some delay (allow
								// time for tester to get focus)
				PollInterval); // run after every specific interval
	}

	// Sequentially test multiple files for multiple runs.
	private void multiRuns(File mexfile[], String inputs[], String expectedOutputs[], IOMode inputMode,
			IOMode outputMode, int maxPollsPerRun) {
		if (mexfile == null || mexfile.length == 0)
			return;
		if (instance == null || robot == null)
			return;
		// The length of inputs determines the number of runs.
	...
	private void verifyOutputs(String expectedOutput) {
		...
		if (testOutput != null) {
			// For example, if whitespaces don't matter, then replace all
	...
	private void singleRun(String masfilepath, String input, String expectedOutput, int maxPollCount, IOMode inputMode,
			IOMode outputMode) {
		if (masfilepath == null)
			return;
	...
	private void multiRuns(File masfile, String inputs[], String expectedOutputs[], int maxPollCount, IOMode inputMode,
			IOMode outputMode) {
		setlog(masfile.getPath().replace(".mas", ".log"));
		File mexfile = assemble(masfile);
		if (mexfile != null) {
	...
	private void multiRuns(File masfiles[], String inputs[], String expectedOutputs[], int maxPollCount,
			IOMode inputMode, IOMode outputMode) {
	...
	private void multiRuns(String masfilepath, String inputs[], String expectedOutputs[], int maxPollCount,
			IOMode inputMode, IOMode outputMode) {
		if (masfilepath == null)
			return;
		File masfile = new File(masfilepath);
	...
	// If a directory is provided, test all mas files in the directory with
	// multiple runs.
