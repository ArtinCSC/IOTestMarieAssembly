...
public class TextFileViewer extends JFrame {
	/******************************************************************************************
	 * This program provides utility functions for reading and printing text
	 * files using * the Java component JEditorPane. Its functionality can be
	 * invoked in any of three * ways: 1. Through a console command: * java
	 * TextFileViewer <Window title> <FileToDisplay> * * 2. Through a call to
	 * the showFile method giving parameters: * * String frameTitle, String
	 * fileName, boolean exitMode * * where exitMode indicates whether the
	 * program should terminate * (System.exit(0)) or just return to its caller.
	 * * * 3. Through a call to the constructor giving parameters: * * String
	 * frameTitle, String fileName, boolean exitMode * * as explained above. * *
	 * If the third method is used, the object created (a subclass of JFrame)
	 * created * can be manipulated by the calling program to change its sizing
	 * and position. * * If the TextFileViewer constructor is called using a
	 * filetype of HTML (".htm"), * the file is loaded directly into a
	 * JEditorPane with no scrollbar at the bottom, * allowing the HTML to wrap
	 * much as it would in a browser. * * If the TextFileViewer constructor is
	 * called using any filetype except HTML, the * file contents are assumed to
	 * be plaintext. This plaintext is wrapped in HTML tags * and rewritten to
	 * an intermediate scratchfile which is then used to call the HTML *
	...
	final static Insets MARGINS = new Insets(5, 5, 10, 5); // top, left, bottom,
															// right
	...
	static JEditorPane printArea = new JEditorPane(); // ... a static image for
														// printing
	...
	public TextFileViewer(String frameTitle, String aTextFile, boolean exitMode) {
		...
		addWindowListener(new WindowAdapter() { // mode.
			public void windowClosing(WindowEvent e) { // Either we quit or
														// return to the
	...
	void getHTMLContent(String aFile) {
		/******************************************************************************************
		 * This method loads the JEditorPane with HTML content found in the
		 * filename passed * as an argument. When plain text (non-HTML) content
		 * needs to be displayed, the * getTextContent() method should be called
		 * first. *
		 ******************************************************************************************/
		try {
	...
	void getTextContent(String aTextFile, Object app) {
		/******************************************************************************************
		 * This method translates a plain text file to a preformatted HTML file
		 * by prepending * and appending the appropriate tags. Once these tags
		 * have been added, the modified * text file is used for input by the
		 * getHTMLContent() method so that the plain text can * be displayed
		 * properly in a JEditorPane. When JEditorPane reads a plain text file,
		 * * it exhibits the same "scroll-to-the-bottom" behavior as JTextPane.
		 * HTML content * prevents this scrolling (!). The temporary file that
		 * is used as input for the HTML * input is deleted before this method
		 * terminates. * * April 2006 Update: The MARIE package has been
		 * modified to allow the simulator to * run from an executable JAR file.
		 * In order to be able to read help * files, etc., from the archive, the
		 * path qualification has to be * removed. However, we still need to be
		 * able to read a qualified * file name for MARIE assembly listings,
		 * etc. This is handled in a * try..catch sequence that first tries to
		 * open straightforwardly * whatever file name is passed to this
		 * routine. If that file cannot * be found, then we attempt to open it
		 * as a "reource," which is how * JAR file contents must be accessed. If
		 * neither one can be opened, * the "file not found" message is
		 * displayed. *
		 ******************************************************************************************/
		...
		File aFile = new File("TextFileViewer.out"); // into the display pane
														// and
	...
	void printContent() {
		/******************************************************************************************
		 * This method calls upon SDK 1.4 printing facilities to print the file
		 * that was passed * to the TextFileViewer. Note: If the filetype is not
		 * supported by the system printer, * some odd-looking output may
		 * result. *
		 ******************************************************************************************/
	...
	public static void showFile(String frameTitle, String aTextFile, boolean exitMode) {
		/******************************************************************************************
		 * This method provides access to the services of this program without
		 * the caller * needing to supply a TextFileViewer object instance. See
	...
	public static void main(String args[]) {
		/******************************************************************************************
		 * This method provides access to the services of this program from the
		 * system command * line. See above for more detail. *
		 ******************************************************************************************/
