...
public class Assembler implements Serializable {
	/****************************************************************************************************
	 * This program assembles code written for the MARIE (Machine Architecture
	 * that is Really * Intuitive and Easy) computer as described in *The
	 * Essentials of Computer Organization and * Architecture* by Null & Lobur.
	 * As with most assemblers, this assembler works in two passes * over the
	 * source code: The first pass performs simple translation of mnemonics to
	 * their * hex equivalent, e.g., LOAD = 1, and adding whatever symbols that
	 * it finds to a symbol table * maintaned by this program. The first pass of
	 * the program creates an intermediate workfile * of serialized
	 * AssembledCodeLine objects, still containing symbolic names instead of
	 * memory * addresses. * * The second pass of the assembler, reads the
	 * intermediate workfile and supplies addresses * for the symbols using
	 * addresses found in the symbol table. (Or producing an error if the *
	 * symbol can't be found.) The output of this phase is written to another
	 * workfile. * * The final output of the assembler, the "MARIE EXecutable"
	 * file is produced from the second- * pass workfile if there are no errors.
	 * A map of symbol addresses from the symbol table is * also produced so
	 * that it can be used as an online reference when running the MARIE
	 * simulator. * A listing file is produced whether or not the assembly was
	 * successful. The output format of * both the assemby listing and symbol
	 * table is HTML to facilitate access within the environment * of the MARIE
	 * simulator. * * * Implementation Notes: 1. Unlike "real" assemblers, this
	 * assembler does not produce binary * machine code (though it would be easy
	 * to make it do so). The * output is instead an object stream that is used
	 * by both the listing- * formatting method (below) and the MARIE simulator
	 * program. The MARIE * simulator is the only place where output code from
	 * this program will * run, so we format our output accordingly. * 2. The
	 * reader may be curious as to why the author chose to use Java * integers
	 * instead of Java short integers, which are exactly the * same size as a
	 * MARIE word. Java integers were used because they * offered the path of
	 * least resistance, eliminating the need for * type casts throughout
	 * programs related to the MARIE system. * This decision meant that some
	 * fancy footwork had to be done when * converting between radices, but
	 * overall fewer difficulties were * presented by using this approach. * * *
	 * June 23, 2010: Added instructions LoadI and StoreI. * * *
	 ****************************************************************************************************/
	/*
	 * Notes: Labels are case sensitive, otherwise codeline is case insensitive.
	 * Hex literals must begin with a digit, e.g., BABE must be 0BABE. Address
	 * literals (instruction operands) must be in hex and in MARIE-addressable
	 * range. Numeric literals must be in the range of -32768 to 32767. MARIE
	 * uses 16 bits only so 0 -> 7FFF = 0 to 32767 and 8000 -> 0 = -32768 to 0.
	 */
	...
	public ObjectOutputStream objFileOut = null; // Object writer for
													// intermediate code.
	public BufferedWriter lstFile = null; // Buffered writer for text output.
	public ObjectInputStream objFileIn = null; // Object reader for intermediate
												// code.
	...
	public static final String sourceType = "mas"; // File exentions: MAS =
													// MARIE Source
	public static final String listType = "lst"; // LST = Assembler listing
													// (text)
	public static final String mapType = "map"; // MAP = Symbol table (text)
	public static final String exeType = "mex"; // MEX = Executable for
												// simulator
	...
	public static final int MAX_SYMBOL_PRINT_LEN = 24; // Maximum size of symbol
														// when printing
														// symbol table. All
														// chars significant,
														// only this many shown.

	public static final int LABEL_DELIM = (int) ','; // Punctuation for a label
														// statement.
	public static final int COMMENT_DELIM = (int) '/'; // Punctuation for inline
														// comment.
	...
	public static final String[] errorMsgs = { "ORiGination directive must be first noncomment line of program ", // 0
	...
	public ArrayList errorList = new ArrayList(); // Holds text of any errors
													// found.
	public boolean operandReqd = false; // Does current instruction need an
										// operand?
	...
	class SymbolEntry {
		/******************************************************************************************
		 * Inner class SymbolEntry is the framework for the objects that are
		 * used to store and * retrieve values that form the Assembler's symbol
		 * table. As these objects are * instantiated, they are placed in a
		 * HashTable structure. As references to the symbols * are found, they
		 * are added to the SymbolEntry's vector of references. *
		 ******************************************************************************************/
	...
	class Instruction implements Serializable {
		/******************************************************************************************
		 * Inner class Instruction stores the important components of a Marie
		 * machine instruction. *
		 ******************************************************************************************/
	...
	/******************************************************************************************
	 * Create searchable instruction set hashtable from a stream of literal
	 * values. The * Instruction objects end up in a Hashtable. The format for
	 * this hashtable is given * in the class definition for Instruction
	 * (above). *
	 ******************************************************************************************/
	...
	String statementLabel(String stmt) {
		/******************************************************************************************
		 * Looks for label punctuation in the parameter String. Returns the
		 * label if found * and calls method to add the symbol to the
		 * instruction table. *
		 ******************************************************************************************/
		String aSymbol = null;
	...
	boolean tokenIsLiteral(String token) {
		/******************************************************************************************
		 * This method determines whether the token passed as a parameter is a
		 * valid hex literal. * If a hex literal is used as an address literal
		 * (as opposed to a symbolic reference to * an address) the address must
		 * begin with a zero, even if it means the literal will be * longer than
		 * 3 characters. (This is the only way we can tell the address A from
		 * the * symbol A.) Note, the check of the token is case insensitive. *
		 ******************************************************************************************/
		char[] tokenChars = token.toCharArray();
		if (!Character.isDigit(tokenChars[0])) // First character of a numeric
			return false; // literal must be 0..9.
		for (int i = 0; i < token.length(); i++) // Now check that the rest of
													// the literal is
	...
	int validMarieValue(int number) {
		/******************************************************************************************
		 * Used by the literalToInt() method to check the value of the parameter
		 * integer with * respect to the 16-bit word size of Marie.
		 * Specifically, Java values in the 32768 to * 65535 (absolute value)
		 * translate to -32768 -> 0 in MARIE memory. Otherwise, anything * out
		 * of the -32768 to 32767 range returns Integer.MAX_VALUE. *
		 ******************************************************************************************/
	...
	int literalToInt(int literalType, String literal, boolean directive) {
		/******************************************************************************************
		 * Converts a String literal to integer. * Parameters: * int literalType
		 * = DEC, OCT, HEX, ORG, and END (final static int constants), * the
		 * String literal to be converted to an integer, and * a boolean to
		 * indicate whether the String literal was found in a directive *
		 * statement, such as OCT or HEX, or whether it was found as an address
		 * literal * in an imperative MARIE assembler statement. * This method
		 * will return Integer.MAX_VALUE to flag any exceptions thrown. (We can
		 * get * away with this because Marie's word size is smaller than
		 * Java's.) *
		 ******************************************************************************************/
	...
	boolean addedToSymbolTable(String symbol) {
		/******************************************************************************************
		 * Returns true if argument symbol (along with the line number where it
		 * is defined) is * successfully added to the symbol table. If the
		 * symbol is already in the symbol table, * this method will return
		 * false. *
		 ******************************************************************************************/
		SymbolEntry se;
		...
		if (symbol.length() > maxSymbolLength) // Get this size for output
												// formatting
	...
	String getSymbolAddress(String symbol, String referenceLine) {
		/******************************************************************************************
		 * Retrieves the address where the symbol is defined from the symbol
		 * table. *
		 ******************************************************************************************/
		SymbolEntry se;
		String address = null;
	...
	String padStr(String s, int size) {
		/******************************************************************************************
		 * Adds trailing blanks to pad the string s to the length (size)
		 * specified in the * argument list, if it is longer than "size."
		 * Truncates if shorter. *
		 ******************************************************************************************/
		int strLen = s.length();
		if (strLen > size)
			strLen = size;
	...
	int getOpcode(String stmt) {
		/******************************************************************************************
		 * Tries to find the argument stmt in the Hashmap instructionSet. If not
		 * found, returns * Java Integer.MIN_VALUE (a number we'd never see in a
		 * MARIE instruction set). * Also makes a "special case" check that an
		 * ORiGination statement must be the first * non-comment line of a MARIE
		 * program. *
		 ******************************************************************************************/
		...
		if (instructionSet.containsKey(stmt)) { // Try to find stmt value in
			instruction = (Instruction) instructionSet.get(stmt); // instruction
																	// set.
	...
	void setErrorMessage(String msg) {
		/******************************************************************************************
		 * Increments the error count, sets the error flag and adds the message
		 * string of the * argument to the list (Vector) of errors for the
		 * current code line being parsed. *
		 ******************************************************************************************/
		errorCount++;
		errorFound = true;
		errorList.add(msg);
	...
	AssembledCodeLine parseCodeLine(String inputLine) {
		/******************************************************************************************
		 * This method controls extraction of symbols from a single line of
		 * source code passed as * a String argument. The return value is an
		 * object composed of the extracted tokens * along with a Vector
		 * containing any error messages found, and the input line itself. * If
		 * the inputLine is a blank line or a comment, the token-related fields
		 * in the returned * object are all spaces, and the object is populated
		 * only with the source code line * itself. * * Side effects: * As
		 * noncomment code lines are processed, the (global) line counter (or
		 * address * value) is incremented. Also, if any errors are found (such
		 * as a missing token), * the error count and global error flag are
		 * updated through calls to the * error handler. *
		 ******************************************************************************************/

		AssembledCodeLine aCodeLine = new AssembledCodeLine(); // Create the
																// output object
		StringBuffer codeLine = new StringBuffer(" "), // String buffer for
														// parsing source code.
				instructionLabel = new StringBuffer(" "), // String buffer for
															// statement label.
				operand = new StringBuffer(" "); // String buffer for the
													// operand.
		int instructionCode = 0, anIntValue = 0;
		...
		if (st.countTokens() == 0) { // If there are no tokens, we have a blank
			aCodeLine.comment = inputLine; // line (or comment). No need to
											// parse it.
			return aCodeLine;
		}
		...
		/*
		 * -- We assume Assume there is no label, the statementLabel() method
		 * sets the --
		 */
		...
		/*
		 * -- Once we have the opcode, we also know whether an operand is
		 * needed. --
		 */
		/* -- -- */
		...
		/* -- -- */
		/*
		 * -- Get the operand. If no operand is needed, we would have found out
		 * when we got --
		 */
		/*
		 * -- the opcode. In the process of finding the opcode, the operandReqd
		 * flag is set --
		 */
		...
		if (operandReqd) {
			if (st.hasMoreTokens()) {
				aToken = st.nextToken();
				if ((instructionCode >= ORG) && (instructionCode < 0)) { // Do
																			// we
																			// have
																			// a
																			// "constant"
					...
					if ((instructionCode > ORG) || (errorFound)) {
						...
						aCodeLine.operandToken = aToken.toUpperCase(); // in
																		// instructionCode
																		// field
					...
					if (tokenIsLiteral(aToken)) { // literal or label.
						...
						aToken = to3CharHexStr(anIntValue).toUpperCase(); // Convert
																			// literal
																			// to
																			// uppercase
	...
	AssembledCodeLine symbolsToAddresses(AssembledCodeLine codeLine) {
		/******************************************************************************************
		 * This is the second pass of the assembler. All we need to do is find
		 * the addresses of * the symbols in the assembler program. We either
		 * find them in the symbol table or we * don't. *
		 ******************************************************************************************/
		...
		if (codeLine.operand.indexOf((int) '_') == 0) {
			currAddress = getSymbolAddress(codeLine.operand.substring(1, codeLine.operand.length()), currAddress);
			if (currAddress != null) {
				codeLine.operand = currAddress;
			} else
				codeLine.operand = "???"; // Error: Symbol not found.
		}
		if (errorFound) { // We have only one possible kind
			codeLine.errors.add((String) errorList.get(0)); // of error from
															// this pass.
		}
		return codeLine;
	} // symbolsToAddresses()
	...
	int performFirstPass() {
		/******************************************************************************************
		 * This method calls methods to open all of the first-pass files, read
		 * the source file, * parse the input, and write the output to the
		 * intermediate output file. When complete, * all related files are
		 * closed. * Note: A negative return value indicates a critical error in
		 * the file handling * only and has nothing to do with any errors found
		 * in the assembly program * code. * * When this pass is complete, the
		 * "binary" program output is ready for second-pass * processing. *
		 ******************************************************************************************/
	...
	int performSecondPass() {
		/******************************************************************************************
		 * This method reads the partially assembled code file produced by the
		 * first pass * and calls the method that supplies addresses for the
		 * symbols in that file. The * The output is <filename>.MEX that is the
		 * MARIE executable used by the MARIE * machine simulator. A file,
		 * <filename>.map, containing the symbol table is also * produced for
		 * the user's reference while running the MARIE simulator. * Note: A
		 * negative return value indicates a critical error in the file handling
		 * * only and has nothing to do with any errors found in the assembly
		 * program * code. *
		 ******************************************************************************************/
		errorFound = false;
		openSecondPassFiles();
	...
	void produceFinalOutput() {
		/******************************************************************************************
		 * This method produces the final outputs from the MARIE assembler,
		 * using the ".MO2" * file as input. An assembly listing, <filename>.LST
		 * is always produced by this step. * If assembly was error-free, a
		 * ".MEX" (MARIE EXecutable) file is produced along with * a
		 * <filename>.MAP file containing the symbol table which can be used for
		 * later * reference when running the simulator. *
		 ******************************************************************************************/
		AssembledCodeLine aCodeLine = new AssembledCodeLine();
		done = false;
		openFinalFiles();
		...
		else if (maxSymbolLength > MAX_SYMBOL_PRINT_LEN) // margin (assuming
															// reasonable font
															// size).
		...
		while (!done) {
			try {
				aCodeLine = (AssembledCodeLine) objFileIn.readObject(); // Print
																		// a
																		// formatted
				if (aCodeLine == null) // line on the listing.
					...
					lstFile.write(" " + padStr(aCodeLine.operandToken, // Put
																		// spaces
																		// after
																		// the
																		// operand...
							maxSymbolLength + (9 - aCodeLine.mnemonic.length())));
					lstFile.write(" " + aCodeLine.comment + lineFeed); // ...so
																		// comments
																		// will
																		// line
																		// up.
					for (int i = 0; i < aCodeLine.errors.size(); i++) // Error
																		// list
																		// prints
						lstFile.write("   **** " + aCodeLine.errors.get(i) + lineFeed); // for
																						// each
																						// line.
				}
			} // try
	...
	void dumpSymbolTable() throws IOException {
		/******************************************************************************************
		 * Place the contents of the symbol table, including line number where
		 * defined and line * numbers where referenced, on the assembly listing.
		 * We attempt to make a nicely- * formatted table, padding out all
		 * symbol names to match the length of the longest name. * The "symbol
		 * name" column will be a minumum of 6 characters and a maximum of *
		 * MAX_SYMBOL_PRINT_LEN. Symbols longer than MAX_SYMBOL_PRINT_LEN will
		 * be truncated for * printing purposes, but the entire symbol name is
		 * significant (up to the limits imposed * by the Java language). * If
		 * assembly was successful, we also write symbol table entries to a
		 * plain text * "mapfile" for later reference (by the progammer) while
		 * the MARIE program is executing. *
		 ******************************************************************************************/
		SymbolEntry se;
	...
	public static int assembleFile(String fileName) {
		/******************************************************************************************
		 * This method is the mainline for the MARIE assembler. It expects to be
		 * passed the * name of a MARIE assembly code file, <filename>, that
		 * will be opened as <filename>.MAS. * Ultimately, a <filename>.LST,
		 * <filename>.MAP and <filename>.MEX files will be created * from the
		 * <filename>.MAS file. *
		 ******************************************************************************************/
		...
		if (fileName == null) { // Make sure we have an
			System.err.println("\nNull input file to assembler."); // input file
																	// specified.
			return -1;
		}
		...
		irrecoverableError = assembler.performFirstPass(); // Call functional
															// methods.
		if (irrecoverableError == 0) // irrecoverableError(s) occur
			irrecoverableError = assembler.performSecondPass(); // as a result
																// of unexpected
	...
	void openFirstPassFiles() {
		/******************************************************************************************
		 * Opens files required by first-pass processing. * The filename,
		 * sourceFileName (class field), has ".MAS" appended to it prior to any
		 * * sourcefile open attempts. An intermediate file <sourceFileName>.MO1
		 * is created to * hold assembled code "objects" used for input by the
		...
		try { // Create the intermediate output code.
			objectFile = new File(sourceFileName + ".MO1"); // MO1 = Marie
															// Object, first
															// pass
	...
	void openSecondPassFiles() {
		/******************************************************************************************
		 * Opens files required by second-pass processing. * We expect to see an
		 * ".M01" file that was created by the first pass. We will write the *
		 * output of this assembler phase to an ".MEX" file, which consists of
		 * assembled MARIE * code lines. *
		 ******************************************************************************************/
		try { // Try to open the input.
	...
	void closeSecondPassFiles() {
		/******************************************************************************************
		 * Closes files opened in the second pass, deleting the workfile
		 * (<fileName>.MO1) created * by the first pass. *
		 ******************************************************************************************/
		try { // Close intermediate file.
	...
	void openFinalFiles() {
		/******************************************************************************************
		 * The "final" assembler phase writes the output to a text listing file
		 * and creating the * "binary" output. Note the objectFile workfile from
		 * the second pass is our input. *
		 ******************************************************************************************/
		...
		try { // Open output and
			objectFile = new File(sourceFileName + "." + exeType); // try to
																	// open the
																	// input.
			objFileIn = new ObjectInputStream(new FileInputStream(objectFile));
		} // try
	...
	void closeFinalFiles() {
		/******************************************************************************************
		 * Closes files opened in the final pass, deleting the "binary"
		 * objectFile if assembly * was unsuccessful. *
		 ******************************************************************************************/
	...
	public static void main(String args[]) {
		/******************************************************************************************
		 * This main method runs the MARIE assembler in standalone console mode
		 * by providing a * hook to the mainline processing method
		 * assembleFile(). We do this so that the * assembler can be used easily
		 * as a class method from another program. *
		 ******************************************************************************************/
		assembleFile(args[0]);
	} // main()
} // Assembler